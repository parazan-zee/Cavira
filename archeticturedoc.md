...
| **Services** | `AppServices` + `Environment` (`AppServices?`); photo library + image loader + import; **`DataService`** incl. **`resetSettingsToDefaults`**, **`deleteAllCaviraData`** (deletes **`HomeCollection`** and clears members per dissolve semantics). **Grid/timeline order:** new collections append via **`HomeAlbumRow.renumberGridTimelineAppendingCollection`** after import. |
...
| **Photo detail** | **`PhotoDetailView`**: principal **date + location** (`PhotoDetailNavChrome`), trailing **⋯** (Edit, Share, Remove / **Delete collection**). **People tags:** horizontal chip row in **`safeAreaInset(edge: .top)`** (leading), not placed on the image. **Share / delete-from-home** in **`PhotoDetailCommandHelpers`**. **Collection pager:** **`isEmbeddedInCollectionPager`**; **no per-page toolbar** — **`HomeCollectionViewer`** owns **`.toolbar`** (principal + **`1 / N`** + **`PhotoDetailPagerOverflowMenu`**). **Delete collection** removes the whole **`HomeCollection`** from Home. |
...
- **People tags:** Shown only as a **top leading** chip strip (`safeAreaInset`); no on-image placement or “place tags” mode. **`PhotoDetailPagerOverflowMenu`** mirrors standalone ⋯ (no placement actions).
- **New collection order:** After import, **`HomeAlbumRow.renumberGridTimelineAppendingCollection`** assigns contiguous **`homeOrderIndex`** values so the new collection is **last** on Grid/Timeline (avoids `nil`-index photos sorting after indexed tiles).
...
- **Chrome:** **`CaviraTheme`** + **`ThemePalette`** (user-selectable under **Settings → Display → Theme**); **`CaviraTheme.applyGlobalChrome()`** at launch and on theme change. Theme picker uses **`themePickerSwatchColor`** so light/dark palettes show correct chip contrast.
